...
protected-path handling defaults to `deny`. `workflow` tools must declare fixed
`steps`. Optional pass-through upstreams remain in the tool surface and return
structured `unavailable` results when the upstream is down. The editor schema at
`policies/schema.json` is generated by `npm run policy:schema`.
