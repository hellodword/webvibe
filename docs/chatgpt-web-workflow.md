...
9. If the best next step requires unavailable arbitrary shell or an unavailable task: show manual instructions, then call `manual.gate` with only `reason` and `hostObservation`
