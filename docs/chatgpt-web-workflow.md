...
`MANUAL_PENDING_REQUIRED` except `diagnostics.health`, `manual.status`, and `manual.resume`.
...
first command token, accepting `/resume <operationId> [workspace-log-path]` and
`/resume cancel <operationId>`, validating any supplied log file exists, and
recording bounded log evidence. Only `confirmed` resumes the original
interrupted request.

Manual continuation sequence:

1. ChatGPT shows complete manual instructions in chat.
2. When possible, `manual.prepare` first saves operationId, original request summary, verification checks, and nextAfterResume metadata without commands, diffs, stdout/stderr, or log contents.
3. `manual.gate` returns an `awaiting_manual_completion` tool result.
4. Assistant stops; if it tries to call more tools, relay returns `MANUAL_PENDING_REQUIRED`.
5. User completes the required manual work outside ChatGPT.
6. User replies with `/resume <operationId>`, `/resume <operationId> .webvibe/manual-logs/<id>.log`, or `/resume cancel <operationId>`.
7. Model calls `manual.resume` with the user's message.
8. `manual.resume` records confirmed/cancelled/expired, validates any supplied log path exists, stores size/sha256/head/tail evidence, and verifies configured checks.
9. Only `confirmed` resumes the original interrupted workflow. `cancelled`, `expired`, `not_found`, and `verification_failed` do not continue the original task.
...
| next user message starts with `/resume` | `manual_completion_resumed` | yes | call `manual.resume`; continue only if status is `confirmed` | 0 | pending record transitions if checks pass or cancel requested; verification failure keeps pending | `manual.resume` audit event |
...
Do not continue the original interrupted request unless `manual.resume` reports `confirmed`.
...
user message should still start with `/resume <operationId>`; after
`manual.resume` returns `confirmed`, the model verifies state and resumes the
